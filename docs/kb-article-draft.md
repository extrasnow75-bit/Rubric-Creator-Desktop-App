...
**A merge that has just landed is not on the Releases page yet.** This is the same mistake with a different cause, and it has happened too: the merge and the publish crossed by six seconds. The **Draft a new release** form resolves `master` to a commit when the *page loads*, so a tab left open from earlier — or opened a moment before the merge finished — will quietly tag whatever `master` was then. Reload the page before publishing, and read the commit under the Target dropdown rather than trusting the word `master`: the short SHA it shows must be the merge you just made. When someone else is doing the merging, wait for them to confirm it has landed rather than tagging the moment they say they have started.
